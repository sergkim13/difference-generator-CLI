...
def format_diff(source, replacer=' ', spacesCount=2):
    ...
    def inner(current_value, depth):
        ...
        for key, val in current_value.items():
            if key[1] == 'added':
                lines.append(f'{deep_indent}+ {key[0]}: {inner(val, deep_indent_size + 2)}')
            elif key[1] == 'deleted':
                lines.append(f'{deep_indent}- {key[0]}: {inner(val, deep_indent_size + 2)}')
            elif key[1] == 'changed':
                lines.append(
                    f'{deep_indent}- {key[0]}: {inner(val[0], deep_indent_size + 2)}\n'
                    f'{deep_indent}+ {key[0]}: {inner(val[1], deep_indent_size + 2)}')
            elif key[1] == 'unchanged':
                lines.append(f'{deep_indent}  {key[0]}: {inner(val, deep_indent_size + 2)}')
            elif key[1] == 'children':
                lines.append(f'{deep_indent}  {key[0]}: {inner(val, deep_indent_size + 2)}')
            else:
                lines.append(f'{deep_indent}  {key}: {inner(val, deep_indent_size + 2)}')
